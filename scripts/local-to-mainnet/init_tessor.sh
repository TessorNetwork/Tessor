...
# CHAIN PARAMS
BLOCK_TIME='5s'
TESSOR_DAY_EPOCH_DURATION="120s"
TESSOR_EPOCH_EPOCH_DURATION="120s"
...
NODE_NAME="tessor1"
TESSOR_VAL_MNEMONIC="close soup mirror crew erode defy knock trigger gather eyebrow tent farm gym gloom base lemon sleep weekend rich forget diagram hurt prize fly"
...
TESSOR_VAL_ACCT=val1
HERMES_ACCT=hrly1
RELAYER_ACCT=rly1

CMD="$SCRIPT_DIR/../../build/tessord --home $SCRIPT_DIR/../state/tessor1"

echo "Initializing Tessor chain..."

# Create a state directory for the current node and initialize the chain
mkdir -p $STATE/$NODE_NAME
$CMD init TESSOR --chain-id $TESSOR_CHAIN_ID --overwrite &> /dev/null
...
sed -i -E "s|chain-id = \"\"|chain-id = \"${TESSOR_CHAIN_ID}\"|g" $client_toml
...
echo "$TESSOR_VAL_MNEMONIC" | $CMD keys add $TESSOR_VAL_ACCT --recover --keyring-backend=test >> $KEYS_LOGS 2>&1
VAL_ADDR=$($CMD keys show $TESSOR_VAL_ACCT --keyring-backend test -a)
...
$CMD gentx $TESSOR_VAL_ACCT ${STAKE_TOKENS}${DENOM} --chain-id $TESSOR_CHAIN_ID --keyring-backend test &> /dev/null
...
# add Hermes and relayer accounts on Tessor
...
echo "$TESSOR_ADMIN_MNEMONIC" | $CMD keys add $TESSOR_ADMIN_ACCT --recover --keyring-backend=test >> $KEYS_LOGS 2>&1
TESSOR_ADMIN_ADDRESS=$($CMD keys show $TESSOR_ADMIN_ACCT --keyring-backend test -a)
$CMD add-genesis-account ${TESSOR_ADMIN_ADDRESS} ${ADMIN_TOKENS}${DENOM}
...
# update params
jq '(.app_state.epochs.epochs[] | select(.identifier=="day") ).duration = $epochLen' --arg epochLen $TESSOR_DAY_EPOCH_DURATION $genesis_json > json.tmp && mv json.tmp $genesis_json
jq '(.app_state.epochs.epochs[] | select(.identifier=="tessor_epoch") ).duration = $epochLen' --arg epochLen $TESSOR_EPOCH_EPOCH_DURATION $genesis_json > json.tmp && mv json.tmp $genesis_json

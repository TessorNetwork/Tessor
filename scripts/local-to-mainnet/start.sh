...
TESSOR_CHAIN_ID=local-test-1
HOST_CHAIN_ID=osmosis-1
HOST_ENDPOINT=osmo-fleet-direct.main.tessornet.co
...
TESSOR_HOME=$STATE/tessor1
DOCKER_COMPOSE="docker-compose -f $SCRIPT_DIR/docker-compose.yml"

HERMES_TESSOR_MNEMONIC="alter old invest friend relief slot swear pioneer syrup economy vendor tray focus hedgehog artist legend antenna hair almost donkey spice protect sustain increase"
RELAYER_TESSOR_MNEMONIC="pride narrow breeze fitness sign bounce dose smart squirrel spell length federal replace coral lunar thunder vital push nuclear crouch fun accident hood need"
...
# Start tessor
bash ${SCRIPT_DIR}/init_tessor.sh $TESSOR_CHAIN_ID

$DOCKER_COMPOSE up -d tessor1
$DOCKER_COMPOSE logs -f tessor1 | sed -r -u "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2})?)?[mGK]//g" > $TESSOR_LOGS 2>&1 &

printf "Waiting for Tessor to start..."
( tail -f -n0 $TESSOR_LOGS & ) | grep -q "finalizing commit of block"
echo "Done"
...
sed -i -E "s|TESSOR_CHAIN_ID|$TESSOR_CHAIN_ID|g" $HERMES_CONFIG_FILE
...
sed -i -E "s|TESSOR_CHAIN_ID|$TESSOR_CHAIN_ID|g" $RELAYER_CONFIG_FILE
...
echo "$HERMES_TESSOR_MNEMONIC" > $TMP_MNEMONICS
$HERMES_CMD keys add --key-name hrly1 --chain $TESSOR_CHAIN_ID --mnemonic-file $TMP_MNEMONICS --overwrite
...
$RELAYER_CMD keys restore tessor rly1 "$RELAYER_TESSOR_MNEMONIC" 
...
sed -i -E "s|TESSOR_HOME|s|g" $COMMANDS_FILE
sed -i -E "s|TESSOR_CHAIN_ID|$TESSOR_CHAIN_ID|g" $COMMANDS_FILE

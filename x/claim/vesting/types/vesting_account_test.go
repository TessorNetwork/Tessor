...
func TestGetVestedCoinsPeriodicVestingAcc(t *testing.T) {
	...
	pva := types.NewTessorPeriodicVestingAccount(bacc, origCoins, periods)
...
func TestTessorPeriodicVestingAccountMarshal(t *testing.T) {
	baseAcc, coins := initBaseAccount()
	now := tmtime.Now()
	acc := types.NewTessorPeriodicVestingAccount(baseAcc, coins, types.Periods{types.Period{now.Unix(), 3600, coins, 0}})
	...
	require.Nil(t, err)
	require.IsType(t, &types.TessorPeriodicVestingAccount{}, acc2)

...
3. `claim_denom` refers to the denomination of claiming tokens. As a default, it's `utess`.
...
For the following commands, you can change `$(tessord keys show -a {your key name})` with the address directly.

Query the claim record for a given address

```sh
tessord query claim claim-record $(tessord keys show -a {your key name})
```
...
tessord query claim claimable-for-action $(tessord keys show -a {your key name}) ActionAddLiquidity
```

Query the total claimable amount that would be earned if all remaining actions were completed right now.

```sh
tessord query claim total-claimable $(tessord keys show -a {your key name}) ActionAddLiquidity
```

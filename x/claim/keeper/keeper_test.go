...
func (suite *KeeperTestSuite) SetupTest() {
	suite.app = app.InitTessorTestApp(true)
	suite.ctx = suite.app.BaseApp.NewContext(false, tmproto.Header{Height: 1, ChainID: "tessor-1", Time: time.Now().UTC()})
	distributors = make(map[string]sdk.AccAddress)

	// Initiate a distributor account for tessor user airdrop

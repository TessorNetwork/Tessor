...
func (suite *KeeperTestSuite) TestSetAirdropAllocationsForMultiAirdrops() {
	...
	totalWeightTessor, err := suite.app.ClaimKeeper.GetTotalWeight(suite.ctx, types.DefaultAirdropIdentifier)
	suite.Require().NoError(err)
	suite.Require().Equal(totalWeightTessor, sdk.NewDecWithPrec(90, 2))

...
func (s *KeeperTestSuite) TestAddValidator_AddressAlreadyExists() {
	...
	badMsg := validMsg
	badMsg.Address = "tessor_VAL1"
	_, err := s.GetMsgServer().AddValidator(sdk.WrapSDKContext(s.Ctx), &badMsg)
	s.Require().EqualError(err, "Validator address (tessor_VAL1) already exists on Host Zone (GAIA): validator already exists")
}

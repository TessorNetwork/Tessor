...
func (k Keeper) DelegateOnHost(ctx sdk.Context, hostZone types.HostZone, amt sdk.Coin, depositRecord recordstypes.DepositRecord) error {
	...
	_, err = k.SubmitTxsTessorEpoch(ctx, connectionId, msgs, *delegationAccount, ICACallbackID_Delegate, marshalledCallbackArgs)
	if err != nil {
...
func (k Keeper) SetWithdrawalAddressOnHost(ctx sdk.Context, hostZone types.HostZone) error {
	...
	_, err = k.SubmitTxsTessorEpoch(ctx, connectionId, msgs, *delegationAccount, "", nil)
	if err != nil {
...
func (k Keeper) UpdateWithdrawalBalance(ctx sdk.Context, hostZone types.HostZone) error {
	...
	ttl, err := k.GetICATimeoutNanos(ctx, epochstypes.TESSOR_EPOCH)
	if err != nil {
		return sdkerrors.Wrapf(sdkerrors.ErrInvalidRequest,
			"Failed to get ICA timeout nanos for epochType %s using param, error: %s", epochstypes.TESSOR_EPOCH, err.Error())
	}
...
func (k Keeper) SubmitTxsTessorEpoch(
	ctx sdk.Context,
	connectionId string,
	msgs []sdk.Msg,
	account types.ICAAccount,
	...
	sequence, err := k.SubmitTxsEpoch(ctx, connectionId, msgs, account, epochstypes.TESSOR_EPOCH, callbackId, callbackArgs)
	if err != nil {
		return 0, err
	}
	return sequence, nil
...
func (k Keeper) QueryValidatorExchangeRate(ctx sdk.Context, msg *types.MsgUpdateValidatorSharesExchRate) (*types.MsgUpdateValidatorSharesExchRateResponse, error) {
	...
	ttl, err := k.GetStartTimeNextEpoch(ctx, epochstypes.TESSOR_EPOCH)
	if err != nil {

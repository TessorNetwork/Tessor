...
func (k msgServer) LiquidStake(goCtx context.Context, msg *types.MsgLiquidStake) (*types.MsgLiquidStakeResponse, error) {
	...
	// tessord tx stakeibc liquid-stake 100 uatom
	...
	tessorEpochTracker, found := k.GetEpochTracker(ctx, epochtypes.TESSOR_EPOCH)
	if !found {
		k.Logger(ctx).Error("failed to find tessor epoch")
		return nil, sdkerrors.Wrapf(sdkerrors.ErrNotFound, "no epoch number for epoch (%s)", epochtypes.TESSOR_EPOCH)
	}
	// Does this use too much gas?
	depositRecord, found := k.RecordsKeeper.GetDepositRecordByEpochAndChain(ctx, tessorEpochTracker.EpochNumber, hostZone.ChainId)
	if !found {
		k.Logger(ctx).Error("failed to find deposit record")
		return nil, sdkerrors.Wrapf(sdkerrors.ErrNotFound, fmt.Sprintf("no deposit record for epoch (%d)", tessorEpochTracker.EpochNumber))
	}

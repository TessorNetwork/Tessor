...
func ValidatorExchangeRateCallback(k Keeper, ctx sdk.Context, args []byte, query icqtypes.Query) error {
	...
	// Get the tessor epoch number
	tessorEpochTracker, found := k.GetEpochTracker(ctx, epochtypes.TESSOR_EPOCH)
	if !found {
		k.Logger(ctx).Error("failed to find tessor epoch")
		return sdkerrors.Wrapf(sdkerrors.ErrNotFound, "no epoch number for epoch (%s)", epochtypes.TESSOR_EPOCH)
	}
	...
	validator.InternalExchangeRate = &types.ValidatorExchangeRate{
		...
		EpochNumber:                tessorEpochTracker.GetEpochNumber(),
	}

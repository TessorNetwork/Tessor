...
func (s *KeeperTestSuite) TestRedeemStake_InvalidCreatorAddress() {
	...
	// cosmos instead of tessor address
	...
	s.Require().EqualError(err, fmt.Sprintf("creator address is invalid: %s. err: invalid Bech32 prefix; expected tessor, got cosmos: invalid address", invalidMsg.Creator))

	// invalid tessor address
	invalidMsg.Creator = "tessor1g6qdx6kdhpf000afvvpte7hp0vnpzapuyxp8uf"
	_, err = s.GetMsgServer().RedeemStake(sdk.WrapSDKContext(s.Ctx), &invalidMsg)
	...
	// wrong len address
	invalidMsg.Creator = "tessor1g6qdx6kdhpf000afvvpte7hp0vnpzapuyxp8ufabc"
	_, err = s.GetMsgServer().RedeemStake(sdk.WrapSDKContext(s.Ctx), &invalidMsg)
...
func (s *KeeperTestSuite) TestRedeemStake_InvalidReceiverAddress() {
	...
	// tessor instead of cosmos address
	invalidMsg.Receiver = "tessor159atdlc3ksl50g0659w5tq42wwer334ajl7xnq"
	_, err := s.GetMsgServer().RedeemStake(sdk.WrapSDKContext(s.Ctx), &invalidMsg)
	s.Require().EqualError(err, "invalid receiver address (invalid Bech32 prefix; expected cosmos, got tessor): invalid address")

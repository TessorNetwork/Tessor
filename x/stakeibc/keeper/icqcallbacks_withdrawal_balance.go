...
func WithdrawalBalanceCallback(k Keeper, ctx sdk.Context, args []byte, query icqtypes.Query) error {
	...
	// Determine the tessor commission rate to the relevant portion can be sent to the fee account
	params := k.GetParams(ctx)
	tessorCommissionInt, err := cast.ToInt64E(params.TessorCommission)
	if err != nil {
		return err
	}

	// check that tessor commission is between 0 and 1
	tessorCommission := sdk.NewDec(tessorCommissionInt).Quo(sdk.NewDec(100))
	if tessorCommission.LT(sdk.ZeroDec()) || tessorCommission.GT(sdk.OneDec()) {
		return sdkerrors.Wrap(sdkerrors.ErrInvalidRequest, "Aborting withdrawal balance callback -- Tessor commission must be between 0 and 1!")
	}

	// Split out the reinvestment amount from the fee amount
	feeAmount := tessorCommission.Mul(sdk.NewDecFromInt(withdrawalBalanceAmount)).TruncateInt()
	...
	_, err = k.SubmitTxsTessorEpoch(ctx, hostZone.ConnectionId, msgs, *withdrawalAccount, ICACallbackID_Reinvest, marshalledCallbackArgs)
	if err != nil {

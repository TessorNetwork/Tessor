...
func (k Keeper) GetTessorEpochElapsedShare(ctx sdk.Context) (sdk.Dec, error) {
	// Get the current tessor epoch
	epochTracker, found := k.GetEpochTracker(ctx, epochstypes.TESSOR_EPOCH)
	if !found {
		errMsg := fmt.Sprintf("Failed to get epoch tracker for %s", epochstypes.TESSOR_EPOCH)
		k.Logger(ctx).Error(errMsg)
...
func (k Keeper) IsWithinBufferWindow(ctx sdk.Context) (bool, error) {
	elapsedShareOfEpoch, err := k.GetTessorEpochElapsedShare(ctx)
	if err != nil {
		return false, err
	}

...
`, version.AppName),
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			tessorDenom, err := sdk.GetBaseDenom()
			if err != nil {
				return err
			}

			if len(deposit) != 1 || deposit.GetDenomByIndex(0) != tessorDenom {
				return sdkerrors.Wrapf(sdkerrors.ErrInvalidCoins, "Deposit token denom must be %s", tessorDenom)
			}

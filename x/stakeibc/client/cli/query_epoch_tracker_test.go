...
func TestShowEpochTracker(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:              "found",
			idEpochIdentifier: tessorEpochId,

			args: common,
			obj:  types.EpochTracker{EpochIdentifier: tessorEpochId, EpochNumber: 1},
		},
		{
			desc:              "not found",
...
func TestListEpochTracker(t *testing.T) {
	...
	expected := []types.EpochTracker{
		...
		{EpochIdentifier: "tessor_epoch", EpochNumber: 1},

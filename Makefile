...
LOCALTESSOR_HOME=./testutil/localtessor
LOCALNET_COMPOSE_FILE=$(LOCALTESSOR_HOME)/localnet/docker-compose.yml
STATE_EXPORT_COMPOSE_FILE=$(LOCALTESSOR_HOME)/state-export/docker-compose.yml
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=tessor \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=tessord \
...
install: go.sum
		go install $(BUILD_FLAGS) ./cmd/tessord
...
stop-docker:
	@pkill -f "docker-compose .*tessor.* logs" | true
...
localnet-keys:
	. $(LOCALTESSOR_HOME)/localnet/add_keys.sh

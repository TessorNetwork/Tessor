...
type AppTestHelper struct {
	...
	TessorChain  *ibctesting.TestChain
...
func (s *AppTestHelper) Setup() {
	s.App = app.InitTessorTestApp(true)
	s.Ctx = s.App.BaseApp.NewContext(false, tmtypes.Header{Height: 1, ChainID: TessorChainID})
...
// Initializes a ibctesting coordinator to keep track of Tessor and a host chain's state
func (s *AppTestHelper) SetupIBCChains(hostChainID string) {
	s.Coordinator = ibctesting.NewCoordinator(s.T(), 0)

	// Initialize a tessor testing app by casting a TessorApp -> TestingApp
	ibctesting.DefaultTestingAppInit = app.InitTessorIBCTestingApp
	s.TessorChain = ibctesting.NewTestChain(s.T(), s.Coordinator, TessorChainID)
	...
	s.Coordinator.Chains = map[string]*ibctesting.TestChain{
		TessorChainID: s.TessorChain,
		hostChainID:   s.HostChain,
	}
	s.IbcEnabled = true
}

// Creates clients, connections, and a transfer channel between tessor and a host chain
func (s *AppTestHelper) CreateTransferChannel(hostChainID string) {
	...
	s.TransferPath = NewTransferPath(s.TessorChain, s.HostChain)
	s.Coordinator.Setup(s.TransferPath)

	// Replace tessor and host apps with those from TestingApp
	s.App = s.TessorChain.App.(*app.TessorApp)
	s.HostApp = s.HostChain.GetSimApp()
	s.Ctx = s.TessorChain.GetContext()

	// Finally confirm the channel was setup properly
	s.Require().Equal(ibctesting.FirstClientID, s.TransferPath.EndpointA.ClientID, "tessor clientID")
	s.Require().Equal(ibctesting.FirstConnectionID, s.TransferPath.EndpointA.ConnectionID, "tessor connectionID")
	s.Require().Equal(ibctesting.FirstChannelID, s.TransferPath.EndpointA.ChannelID, "tessor transfer channelID")
...
func (s *AppTestHelper) CreateICAChannel(owner string) string {
	...
	icaPath := NewIcaPath(s.TessorChain, s.HostChain)
...
// Modifies sdk config to have tessor address prefixes (used for non-keeper tests)
func SetupConfig() {
	app.SetupConfig()
}

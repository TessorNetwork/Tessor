...
# Contants
BONDED_TOKENS_POOL_MODULE_ADDRESS = "tessor1fl48vsnmsdzcv85q5d2q4z5ajdha8yu3ksfndm"

config = {
    "governance_voting_period": "180s",
    "epoch_day_duration": '3600s',
    "epoch_tessor_duration": "3600s",
}
...
def main():
    ...
    old_validator = Validator(
        ...
        operator_address = "tessorvaloper1h2r2k24349gtx7e4kfxxl8gzqz8tn6zym65uxc",
        consensus_address = "tessorvalcons197q3lkd66vl89fn5mj5c590t4ujpxsd8rus25g"
    )
    ...
    old_account = Account(
        ...
        address = "tessor1h2r2k24349gtx7e4kfxxl8gzqz8tn6zyc0sq2a"
    )
    ...
    for epoch in genesis['app_state']['epochs']['epochs']:
        ...
        elif epoch['identifier'] == "tessor_epoch":
            epoch['duration'] = config["epoch_tessor_duration"]
        ...
        if balance['address'] == new_account.address:
            for coin in balance['coins']:
                if coin['denom'] == "utess":
                    ...
                    print("\tUpdate {} utess balance to {}".format(new_account.address, coin["amount"]))
                    break
            break

    # Add 1 BN utess to bonded_tokens_pool module address
    for balance in genesis['app_state']['bank']['balances']:
        if balance['address'] == BONDED_TOKENS_POOL_MODULE_ADDRESS:
            ...
            for coin in balance['coins']:
                if coin['denom'] == "utess":
                    coin["amount"] = str(int(coin["amount"]) + 1000000000000000)
                    print("\tUpdate {} (bonded_tokens_pool_module) utess balance to {}".format(BONDED_TOKENS_POOL_MODULE_ADDRESS, coin["amount"]))
                    break
            break

    # Update bank balance
    for supply in genesis['app_state']['bank']['supply']:
        if supply["denom"] == "utess":
            print("\tUpdate total utess supply from {} to {}".format(supply["amount"], str(int(supply["amount"]) + 2000000000000000)))

...
while true; do
    N_VALIDATORS_TESSOR=$($TESSOR_MAIN_CMD q tendermint-validator-set | grep -o address | wc -l | tr -dc '0-9')
    echo "TESSOR @ $($TESSOR_MAIN_CMD q tendermint-validator-set | head -n 1 | tr -dc '0-9') | $N_VALIDATORS_TESSOR VALS" >$TEMP_LOGS_DIR/$STATE_LOG
    echo "TESSOR @ $($TESSOR_MAIN_CMD q tendermint-validator-set | head -n 1 | tr -dc '0-9') | $N_VALIDATORS_TESSOR VALS" >$TEMP_LOGS_DIR/$BALANCES_LOG
    ...
    printf '\n%s\n' "LIST-HOST-ZONES TESSOR" >>$TEMP_LOGS_DIR/$STATE_LOG
    $TESSOR_MAIN_CMD q stakeibc list-host-zone >>$TEMP_LOGS_DIR/$STATE_LOG
    printf '\n%s\n' "LIST-DEPOSIT-RECORDS" >>$TEMP_LOGS_DIR/$STATE_LOG
    $TESSOR_MAIN_CMD q records list-deposit-record  >> $TEMP_LOGS_DIR/$STATE_LOG
    printf '\n%s\n' "LIST-EPOCH-UNBONDING-RECORDS" >>$TEMP_LOGS_DIR/$STATE_LOG
    $TESSOR_MAIN_CMD q records list-epoch-unbonding-record  >> $TEMP_LOGS_DIR/$STATE_LOG
    printf '\n%s\n' "LIST-USER-REDEMPTION-RECORDS" >>$TEMP_LOGS_DIR/$STATE_LOG
    $TESSOR_MAIN_CMD q records list-user-redemption-record >> $TEMP_LOGS_DIR/$STATE_LOG

    printf '\n%s\n' "BALANCES TESSOR" >>$TEMP_LOGS_DIR/$BALANCES_LOG
    $TESSOR_MAIN_CMD q bank balances $(TESSOR_ADDRESS) >>$TEMP_LOGS_DIR/$BALANCES_LOG

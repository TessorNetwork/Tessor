...
for chain in ${HOST_CHAINS[@]}; do
    ...
    printf "TESSOR <> $chain - Adding relayer keys..."
    $relayer_exec rly keys restore tessor $RELAYER_TESSOR_ACCT "$mnemonic" >> $relayer_logs 2>&1
    ...
    printf "TESSOR <> $chain - Creating client, connection, and transfer channel..." | tee -a $relayer_logs
    $relayer_exec rly transact link tessor-${chain_name} >> $relayer_logs 2>&1
    echo "Done."

...
set_tessor_genesis() {
    ...
    # update params
    jq '(.app_state.epochs.epochs[] | select(.identifier=="day") ).duration = $epochLen' --arg epochLen $TESSOR_DAY_EPOCH_DURATION $genesis_config > json.tmp && mv json.tmp $genesis_config
    jq '(.app_state.epochs.epochs[] | select(.identifier=="tessor_epoch") ).duration = $epochLen' --arg epochLen $TESSOR_EPOCH_EPOCH_DURATION $genesis_config > json.tmp && mv json.tmp $genesis_config
...
for (( i=1; i <= $NUM_NODES; i++ )); do
    # Node names will be of the form: "tessor-node1"
    node_name="${NODE_PREFIX}${i}"
    # Moniker is of the form: TESSOR_1
...
if [ "$CHAIN" == "TESSOR" ]; then 
    # add the tessor admin account
    echo "$TESSOR_ADMIN_MNEMONIC" | $MAIN_CMD keys add $TESSOR_ADMIN_ACCT --recover --keyring-backend=test >> $KEYS_LOGS 2>&1
    TESSOR_ADMIN_ADDRESS=$($MAIN_CMD keys show $TESSOR_ADMIN_ACCT --keyring-backend test -a)
    $MAIN_CMD add-genesis-account ${TESSOR_ADMIN_ADDRESS} ${ADMIN_TOKENS}${DENOM}
...
if [ "$CHAIN" == "TESSOR" ]; then 
    set_tessor_genesis $MAIN_GENESIS
else

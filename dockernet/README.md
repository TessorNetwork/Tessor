...
```
  {new-host-zone}1:
    image: tessorzone:{new-host-zone}
    volumes:
  ...
  relayer-{chain_id}:
    ...
    restart: always
    command: [ "bash", "start.sh", "tessor-{chain_id}" ]
```
...
# tessor1muwz5er4wq7svxnh5dgn2tssm92je5dwthxl7q
...
make start-docker
```
* After the chain is running, run the integration tests to confirm the new host zone is compatible with Tessor
```

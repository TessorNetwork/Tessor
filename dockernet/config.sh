...
#  - OSMO
#  - STARS
#  - HOST (Tessor chain enabled as a host zone)
...
IBC_TESS_DENOM='ibc/FF6C2E86490C1C4FBBD24F55032831D2415B9D7882F85C3CC9C2401D79362BEA'  
...
# CHAIN PARAMS
BLOCK_TIME='1s'
TESSOR_DAY_EPOCH_DURATION="100s"
TESSOR_EPOCH_EPOCH_DURATION="40s"
...
# TESSOR 
TESSOR_CHAIN_ID=TESSOR
TESSOR_NODE_PREFIX=tessor
TESSOR_NUM_NODES=3
TESSOR_VAL_PREFIX=val
TESSOR_DENOM=$TESS_DENOM
TESSOR_RPC_PORT=26657
TESSOR_ADMIN_ACCT=admin
TESSOR_ADMIN_ADDRESS=tessor1u20df3trc2c2zdhm8qvh2hdjx9ewh00sv6eyy8
TESSOR_ADMIN_MNEMONIC="tone cause tribe this switch near host damage idle fragile antique tail soda alien depth write wool they rapid unfold body scan pledge soft"
TESSOR_FEE_ADDRESS=tessor1czvrk3jkvtj8m27kqsqu2yrkhw3h3ykwj3rxh6

# Binaries are contigent on whether we're doing an upgrade or not
if [[ "$UPGRADE_NAME" == "" ]]; then 
  TESSOR_BINARY="$DOCKERNET_HOME/../build/tessord"
else
  TESSOR_BINARY="$UPGRADES/binaries/tessord1"
fi
TESSOR_MAIN_CMD="$TESSOR_BINARY --home $DOCKERNET_HOME/state/${TESSOR_NODE_PREFIX}1"
...
# HOST (Tessor running as a host zone)
HOST_CHAIN_ID=HOST
HOST_NODE_PREFIX=host
HOST_NUM_NODES=1
HOST_BINARY="$DOCKERNET_HOME/../build/tessord"
HOST_VAL_PREFIX=hval
HOST_ADDRESS_PREFIX=tessor
HOST_REV_ACCT=hrev1
...
HOST_RECEIVER_ADDRESS='tessor1trm75t8g83f26u4y8jfds7pms9l587a7q227k9'
...
TESSOR_ADDRESS() { 
  $TESSOR_MAIN_CMD keys show ${TESSOR_VAL_PREFIX}1 --keyring-backend test -a 
}
GAIA_ADDRESS() { 
...
GET_ICA_ADDR() {
  ...
  $TESSOR_MAIN_CMD q stakeibc show-host-zone $chain_id | grep ${ica_type}_account -A 1 | grep address | awk '{print $2}'
}

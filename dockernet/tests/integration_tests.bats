...
setup_file() {
  ...
  TESSOR_VAL=${TESSOR_VAL_PREFIX}1

  TESSOR_TRANFER_CHANNEL="channel-${TRANSFER_CHANNEL_NUMBER}"
...
@test "[INTEGRATION-BASIC-$CHAIN_NAME] host zones successfully registered" {
  run $TESSOR_MAIN_CMD q stakeibc show-host-zone $HOST_CHAIN_ID
...
##############################################################################################
######                TEST BASIC TESSOR FUNCTIONALITY                                   ######
##############################################################################################


@test "[INTEGRATION-BASIC-$CHAIN_NAME] ibc transfer updates all balances" {
  # get initial balances
  sval_tess_balance_start=$($TESSOR_MAIN_CMD  q bank balances $(TESSOR_ADDRESS) --denom $TESSOR_DENOM   | GETBAL)
  hval_tess_balance_start=$($HOST_MAIN_CMD    q bank balances $HOST_VAL_ADDRESS --denom $IBC_TESS_DENOM | GETBAL)
  sval_token_balance_start=$($TESSOR_MAIN_CMD q bank balances $(TESSOR_ADDRESS) --denom $HOST_IBC_DENOM | GETBAL)
  hval_token_balance_start=$($HOST_MAIN_CMD   q bank balances $HOST_VAL_ADDRESS --denom $HOST_DENOM     | GETBAL)

  # do IBC transfer
  $TESSOR_MAIN_CMD tx ibc-transfer transfer transfer $TESSOR_TRANFER_CHANNEL $HOST_VAL_ADDRESS ${TRANSFER_AMOUNT}${TESSOR_DENOM} --from $TESSOR_VAL -y &
  $HOST_MAIN_CMD   tx ibc-transfer transfer transfer $HOST_TRANSFER_CHANNEL  $(TESSOR_ADDRESS) ${TRANSFER_AMOUNT}${HOST_DENOM} --from $HOST_VAL -y &

  WAIT_FOR_BLOCK $TESSOR_LOGS 8

  # get new balances
  sval_tess_balance_end=$($TESSOR_MAIN_CMD  q bank balances $(TESSOR_ADDRESS) --denom $TESSOR_DENOM   | GETBAL)
  hval_tess_balance_end=$($HOST_MAIN_CMD    q bank balances $HOST_VAL_ADDRESS --denom $IBC_TESS_DENOM | GETBAL)
  sval_token_balance_end=$($TESSOR_MAIN_CMD q bank balances $(TESSOR_ADDRESS) --denom $HOST_IBC_DENOM | GETBAL)
  hval_token_balance_end=$($HOST_MAIN_CMD   q bank balances $HOST_VAL_ADDRESS --denom $HOST_DENOM     | GETBAL)

  # get all TESS balance diffs
  sval_tess_balance_diff=$(($sval_tess_balance_start - $sval_tess_balance_end))
  hval_tess_balance_diff=$(($hval_tess_balance_start - $hval_tess_balance_end))
  assert_equal "$sval_tess_balance_diff" "$TRANSFER_AMOUNT"
  assert_equal "$hval_tess_balance_diff" "-$TRANSFER_AMOUNT"
...
@test "[INTEGRATION-BASIC-$CHAIN_NAME] liquid stake mint and transfer" {
  # get initial balances on tessor account
  token_balance_start=$($TESSOR_MAIN_CMD   q bank balances $(TESSOR_ADDRESS) --denom $HOST_IBC_DENOM | GETBAL)
  sttoken_balance_start=$($TESSOR_MAIN_CMD q bank balances $(TESSOR_ADDRESS) --denom st$HOST_DENOM   | GETBAL)
  ...
  # liquid stake
  $TESSOR_MAIN_CMD tx stakeibc liquid-stake $STAKE_AMOUNT $HOST_DENOM --from $TESSOR_VAL -y 

  # sleep two block for the tx to settle on tessor
  WAIT_FOR_STRING $TESSOR_LOGS "\[MINT ST ASSET\] success on $HOST_CHAIN_ID"
  WAIT_FOR_BLOCK $TESSOR_LOGS 2

  # make sure IBC_DENOM went down
  token_balance_end=$($TESSOR_MAIN_CMD q bank balances $(TESSOR_ADDRESS) --denom $HOST_IBC_DENOM | GETBAL)
  ...
  sttoken_balance_end=$($TESSOR_MAIN_CMD q bank balances $(TESSOR_ADDRESS) --denom st$HOST_DENOM | GETBAL)
...
@test "[INTEGRATION-BASIC-$CHAIN_NAME] tokens on $CHAIN_NAME were staked" {
  ...
  WAIT_FOR_STRING $TESSOR_LOGS "\[DELEGATION\] success on $HOST_CHAIN_ID"
  WAIT_FOR_BLOCK $TESSOR_LOGS 2
...
@test "[INTEGRATION-BASIC-$CHAIN_NAME] redemption works" {
  ...
  # call redeem-stake
  $TESSOR_MAIN_CMD tx stakeibc redeem-stake $REDEEM_AMOUNT $HOST_CHAIN_ID $HOST_RECEIVER_ADDRESS \
      --from $TESSOR_VAL --keyring-backend test --chain-id $TESSOR_CHAIN_ID -y

  WAIT_FOR_STRING $TESSOR_LOGS "\[REDEMPTION] completed on $HOST_CHAIN_ID"
  WAIT_FOR_BLOCK $TESSOR_LOGS 2
...
@test "[INTEGRATION-BASIC-$CHAIN_NAME] claimed tokens are properly distributed" {
  ...
  EPOCH=$($TESSOR_MAIN_CMD q records list-user-redemption-record  | grep -Fiw 'epoch_number' | head -n 1 | grep -o -E '[0-9]+')

  # claim the record (send to tessor address)
  $TESSOR_MAIN_CMD tx stakeibc claim-undelegated-tokens $HOST_CHAIN_ID $EPOCH $(TESSOR_ADDRESS) \
    --from $TESSOR_VAL --keyring-backend test --chain-id $TESSOR_CHAIN_ID -y

  WAIT_FOR_STRING $TESSOR_LOGS "\[CLAIM\] success on $HOST_CHAIN_ID"
  WAIT_FOR_BLOCK $TESSOR_LOGS 2
...
@test "[INTEGRATION-BASIC-$CHAIN_NAME] rewards are being reinvested, exchange rate updating" {
  ...
  MULT=1000000
  redemption_rate=$($TESSOR_MAIN_CMD q stakeibc show-host-zone $HOST_CHAIN_ID | grep -Fiw 'redemption_rate' | grep -Eo '[+-]?[0-9]+([.][0-9]+)?')
...
@test "[INTEGRATION-BASIC-$CHAIN_NAME] revenue accrued, and clear-balance works" {
  ...
  $TESSOR_MAIN_CMD tx stakeibc clear-balance $HOST_CHAIN_ID 1 $HOST_TRANSFER_CHANNEL --from $TESSOR_ADMIN_ACCT -y
  WAIT_FOR_BLOCK $TESSOR_LOGS 8

  # check that balance went to revenue account
  fee_tessor_balance=$($TESSOR_MAIN_CMD q bank balances $TESSOR_FEE_ADDRESS --denom $HOST_IBC_DENOM | GETBAL)
  assert_equal "$fee_tessor_balance" "1"
}

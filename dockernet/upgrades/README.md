...
## Run Instructions
* Before working on the upgrade logic, compile the original binary and place it in `dockernet/upgrades/binaries/` named `tessord1`
...
rm -f dockernet/upgrades/binaries/tessord1
cp build/tessord dockernet/upgrades/binaries/tessord1
...
* Then startup the chain as normal and rebuild tessor
```
...
* The startup script will:
    ...
    * Rebuild and replace the tessor docker image with an image that has both binaries and is running cosmosvisor
...
```
* View the tessor logs - you should notice an update occuring at the specified upgrade height.
* After the upgrade has occured, check a post-upgrade condition using `dockernet/upgrades/binaries/tessord2`
...
* Check the tessor logs to confirm the upgrade passes successfully
* Modify `TESSOR_BINARY` in `config.sh` to point to the **new** binary (`TESSOR_BINARY="$UPGRADES/binaries/tessord2"`)
